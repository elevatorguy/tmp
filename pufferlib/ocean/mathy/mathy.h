//Elementary mathematics - 4x4 multiplication
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include "raylib.h"
 ...
 */
void compute_observations(Math* env) {
    int obs_idx = 0;
    for (int a=0; a<env->num_agents; a++) {
        Agent* agent = &env->agents[a];
        for (int g=0; g<env->num_goals; g++) {
...
void c_render(Math* env) {
    if (env->client == NULL) {
        InitWindow(env->width, env->height, "PufferLib Math");
        SetTargetFPS(60);
        env->client = (Client*)calloc(1, sizeof(Client));

        // Don't do this before calling InitWindow
        env->client->puffer = LoadTexture("resources/shared/puffers_128.png");
        env->client->star = LoadTexture("resources/mathy/star.png");
    }
